...
params.ys1[0] = numpy.matmul(params.C1, params.xs[:,0]) + meas_noise_dist # measure from actual plant
...
for t in range(1,params.N):
    ...
    meas_noise_dist = numpy.random.normal(0, numpy.sqrt(params.R1[0]))
    params.ys1[t] = numpy.matmul(params.C1, params.xs[:, t]) + meas_noise_dist # measured from actual plant
